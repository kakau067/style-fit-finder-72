...
export function TryOnOverlay({ request, onClose }: { request: TryOnRequest; onClose: () => void }) {
  ...
  const start = useCallback(async () => {
    ...
    let timedOut = false;
    const timer = window.setTimeout(() => { timedOut = true; controller.abort(); }, 72_000);
    ...
    try {
      ...
      if (timedOut) setError("A geração demorou além do limite. Tente novamente.");
      else if (!controller.signal.aborted) setError(cause instanceof Error ? cause.message : "A prova visual falhou.");
    } finally {
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_280px]">
          <div>
            <div className="relative aspect-[3/4] overflow-hidden rounded-lg border border-line bg-secondary">
              ...
              </> : <><img src={request.photoDataUrl} alt="Sua foto original" className="absolute inset-0 size-full object-cover" /><div className="absolute inset-x-0 bottom-0 bg-background/90 px-4 py-3 text-center font-mono text-xs uppercase tracking-[0.15em] text-foreground">{running ? "Preparando a prova visual…" : error ? "Sua foto original está preservada" : "Compondo a imagem…"}</div></>}
            </div>
