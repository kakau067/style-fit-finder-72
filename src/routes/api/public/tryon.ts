...
async function findProduct(id: string): Promise<Product | null> {
  // Demo products ship with build-time asset references. Prefer those references
  // so stale/legacy database image paths cannot break the public fitting room.
  const bundled = PRODUCT_BY_ID.get(id);
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      void (async () => {
        ...
        const failure = (message: string) => emit(encoder.encode(`event: error\ndata: ${JSON.stringify({ type: "error", error: { message } })}\n\n`));
        try {
          ...
          if (!upstream.ok) failure(`Serviço de prova visual respondeu ${upstream.status}: ${(await upstream.text().catch(() => "")).slice(0, 250)}`);
          else if (upstream.headers.get("content-type")?.includes("text/event-stream") && upstream.body) {
            const reader = upstream.body.getReader();
            while (true) { const { done, value } = await reader.read(); if (done) break; emit(value); }
          } else {
        ...
        } catch (cause) { failure(cause instanceof Error ? cause.message : "A geração foi interrompida."); }
        finally { clearInterval(keepAlive); if (!closed) controller.close(); }
      })();
    },
  });
...
export const Route = createFileRoute("/api/public/tryon")({
  server: { handlers: {
    GET: async ({ request }) => {
      ...
      try { return Response.json(await pollFalTryOn(key, id, model, responseUrl), { headers: { "Cache-Control": "no-store" } }); }
      catch (cause) { return new Response(cause instanceof Error ? cause.message : "Falha ao acompanhar a prova.", { status: 502 }); }
    },
    POST: async ({ request }) => {
      ...
      try {
        ...
        if (selectedProvider === "fal" || (selectedProvider === "auto" && apiKey)) {
          ...
          const queued = await submitFalTryOn(apiKey, {
            ...
            garment_photo_type: "auto", output_format: renderMode === "fast" ? "jpeg" : "png",
          });
          return Response.json({ provider: "fal_queue", ...queued }, { status: 202, headers: { "Cache-Control": "no-store" } });
        }
        const edit = new FormData();
        edit.set("image[]", photo); edit.append("image[]", garment); edit.set("stream", "true");
        const scope = {
