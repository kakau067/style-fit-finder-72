...
export const Route = createFileRoute("/")({
  head: () => ({ meta: [
    ...
    { name: "description", content: "Envie uma foto de corpo inteiro, confira suas medidas e receba o tamanho certo de cada peça, com prova visual gerada na hora." },
    { property: "og:title", content: "Provador Virtual — tamanho certo pela sua foto" },
    { property: "og:description", content: "Recomendação de tamanho e estilo a partir de uma foto, com prova visual das peças no seu corpo." },
...
function Index() {
  ...
  // Preserve merchant/database metadata, but never let stale legacy paths replace
  // the build-time images of the bundled demonstration products.
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b border-line">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-5 py-5 sm:px-8">
          <div><Eyebrow>Ateliê digital</Eyebrow><p className="font-serif text-2xl leading-none text-foreground">Provador Virtual</p></div>
      ...
      <main className="mx-auto max-w-6xl px-5 py-10 sm:px-8 sm:py-14">
        ...
        {stage === 2 && photo ? <ResultsStep ranked={ranked} productsById={productsById} sizesById={sizesById} onTryOn={handleTryOn} onBack={() => setStage(1)} onRestart={() => { setPhoto(null); setBody(DEFAULT_BODY); setProfile(DEFAULT_PROFILE); setStage(0); }} /> : null}
      </main>
      ...
      <footer className="border-t border-line">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3 px-5 py-6 text-xs text-muted-foreground sm:px-8">
          <span>As estimativas e provas visuais são geradas por IA e servem como referência de caimento.</span>
          <Link to="/admin-lojista" className="focus-clay rounded-full font-mono text-[11px] uppercase tracking-[0.2em] text-secondary-foreground underline underline-offset-4">Área do lojista</Link>
        </div>
      </footer>
